...
export default function RequisitionPage() {
    ...
    const fetchShelters = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // ตรวจสอบและสร้าง id สำหรับศูนย์ที่ไม่มี id
          const sheltersWithIds = (data || []).map((shelter: any, index: number) => {
            // ถ้าไม่มี id หรือ id เป็น undefined/null ให้สร้าง id จาก index
            if (!shelter.id || shelter.id === undefined || shelter.id === null) {
              console.warn(`Shelter "${shelter.name}" missing id, assigning id: ${index + 1}`);
              return {
                ...shelter,
                id: index + 1
              };
  ...
  useEffect(() => {
    const loadInventory = async () => {
      try {
        ...
        if (!windowWithStorage.storage) {
          console.warn('window.storage is not available, using default inventory');
          setInventory(defaultInventory);
          return;
        }

        const result = await windowWithStorage.storage.get('ems_inventory');
        
        if (result?.value) {
          setInventory(JSON.parse(result.value));
        } else {
          setInventory(defaultInventory);
          await windowWithStorage.storage.set('ems_inventory', JSON.stringify(defaultInventory));
        }
      } catch (e) {
  ...
  const importToCart = () => {
    ...
    excelPreview.forEach(item => {
      const safeQty = Math.max(0, Math.min(item.limit, Math.min(item.stock, item.stock)));
      if (safeQty > 0) {
  ...
  const downloadSampleExcel = () => {
    const sampleData = [
      { 'รายการ': 'น้ำดื่ม (แพ็ค)', 'จำนวน': 10 },
      { 'รายการ': 'ผ้าห่ม', 'จำนวน': 5 }
    ];
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!windowWithStorage.storage) {
        alert('Storage API is not available. Please use this in Claude Artifacts environment.');
        return;
      }
      ...
      for (const [itemId, qty] of Object.entries(cart)) {
        ...
        if (item && item.stock < totalNeeded) {
          alert(
            `สินค้า "${item.name}" มีสต็อกไม่เพียงพอสำหรับ ${shelterCount} ศูนย์ ` +
            `(ต้องการ ${totalNeeded} แต่มี ${item.stock})`
          );
          return;
        }
      }
      ...
      // บันทึกข้อมูลลงใน persistent storage
      const existingRequests = await windowWithStorage.storage.get('ems_requests');
      const allRequests = existingRequests?.value
        ? JSON.parse(existingRequests.value)
        : [];

      await windowWithStorage.storage.set(
        'ems_requests',
        JSON.stringify([...newRequests, ...allRequests])
      );
      await windowWithStorage.storage.set(
        'ems_inventory',
      ...
      alert(
        `บันทึกใบเบิกเรียบร้อย! (เบิกจ่ายให้ทั้งหมด ${shelterCount} ศูนย์พักพิง)`
      );
      router.push('/');
    } catch (e) {
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              <button
                ...
                className="p-3 rounded-xl bg-slate-800/50 hover:bg-slate-700 border border-slate-700 text-slate-300"
              >
              ...
              <div key={`step-${s}`} className="flex items-center gap-4">
                <div
                  className={`flex items-center gap-3 ${
                    step >= s ? 'text-emerald-400' : 'text-slate-600'
                  }`}
                >
                  <div
                ...
                {idx < 2 && (
                  <div className="w-16 h-[2px] bg-slate-700 relative rounded-full overflow-hidden">
                    <div
            ...
            <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="relative mb-6 max-w-xl mx-auto w-full">
                ...
                <input
                  ...
                  className="w-full bg-slate-900/60 border border-slate-700 rounded-2xl py-4 pl-12 pr-4 text-white outline-none focus:ring-2 focus:ring-emerald-500/50"
                  value={searchTerm}
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-32">
                {loadingShelters ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {filteredShelters.map((shelter, shelterIndex) => {
                      const shelterId = String(shelter.id || shelterIndex);
                      const isSelected = selectedShelters.includes(shelterId);

                      return (
                        <div
                          key={`shelter-${shelter.id}-${shelterIndex}`}
                          onClick={() => {
                          ...
                          return (
                            <div
                              ...
                              className={`p-5 rounded-2xl bg-slate-900/40 border transition-all ${
                                qty > 0
                                  ? 'border-emerald-500/50 bg-emerald-900/5 shadow-md'
                                  : 'border-slate-800 hover:border-slate-700'
                              }`}
                            >
                              <div className="flex items-start justify-between mb-4">
                                <div className="w-12 h-12 rounded-xl bg-slate-800 flex items-center justify-center text-2xl border border-white/5">
                                  {item.image}
                                </div>
                              ...
                              <p className="text-[10px] text-slate-500 mb-4 uppercase tracking-wider">
                                โควตา: {item.limit} / ศูนย์
                              </p>
                              <input
                                type="number"
                                value={qty === 0 ? '' : qty}
                                placeholder="ใส่จำนวน/ศูนย์"
                                onChange={(e) =>
                                  handleInputChange(
                                    item.id,
                                    e.target.value,
                                ...
                                className="w-full bg-slate-900/50 border border-slate-700 rounded-xl py-2 px-3 text-center font-mono font-bold text-white outline-none focus:border-emerald-500"
                              />
                              {qty > 0 && (
                                <p className="text-[10px] text-amber-500 mt-2 text-center font-bold">
                                  รวมเบิกทั้งสิ้น: {totalQty} {item.unit}
                                </p>
                              )}
                            </div>
                          );
                    ...
                    <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/5">
                      <div>
                        <h3 className="text-lg font-bold text-white flex items-center gap-2 font-sans">
                          ...
                          ตรวจสอบรายการจาก Excel
                        </h3>
                        <p className="text-xs text-slate-500">
                          จำนวนนี้จะถูกจ่ายให้ทั้ง {selectedShelters.length} ศูนย์
                        </p>
                      </div>
                      {excelPreview.length > 0 && (
                        <button
                          onClick={importToCart}
                          className="flex items-center gap-2 px-6 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-white rounded-xl text-sm font-bold shadow-lg"
                        >
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-3 custom-scrollbar">
                      {excelPreview.length === 0 ? (
                        ...
                        excelPreview.map((item, idx) => (
                          <div
                            key={`excel-preview-${item.id}-${idx}`}
                            className="flex items-center justify-between p-4 bg-slate-800/50 border border-white/5 rounded-2xl"
                          >
                            <div className="flex items-center gap-4">
                            ...
                            <div className="text-right">
                              ...
                              <p className="text-[10px] text-amber-500 font-bold uppercase">
                                รวม: {item.stock * selectedShelters.length}{' '}
                                {item.unit}
                              </p>
                            </div>
                          </div>
                ...
                {uploadMode ? (
                    ...
                    <div className="relative group border-2 border-dashed border-slate-700 hover:border-emerald-500 bg-slate-900/40 rounded-3xl p-8 transition-all flex flex-col items-center gap-4 text-center">
                      ...
                      <div>
                        <p className="text-white font-bold text-sm">
                          ลากไฟล์ Excel วางที่นี่
                        </p>
                      </div>
                    </div>
                    <button
                      onClick={downloadSampleExcel}
                      className="w-full py-3 rounded-xl bg-slate-800/40 text-slate-400 text-xs font-bold border border-white/5 flex items-center justify-center gap-2"
                    >
                      <FileSpreadsheet className="w-4 h-4" /> ตัวอย่างไฟล์
                    </button>
                  </div>
                ) : (
                  <div className="flex-1 flex flex-col min-h-0 bg-slate-900/40 border border-white/5 rounded-3xl overflow-hidden">
                    <div className="p-4 border-b border-white/5 flex items-center justify-between bg-white/5">
                      <h3 className="font-bold text-xs text-white uppercase tracking-wider flex items-center gap-2 font-mono">
                        <ShoppingCart className="w-4 h-4 text-emerald-500" />{' '}
                        ตะกร้า (ต่อศูนย์)
                      </h3>
                      <span className="text-[10px] px-2 py-1 bg-emerald-500 text-white rounded-md font-black">
                      ...
                      {Object.keys(cart).length === 0 ? (
                        ...
                        Object.entries(cart).map(([itemId, qty]) => {
                          ...
                          return (
                            <div
                              key={`cart-${itemId}`}
                              className="flex items-center justify-between p-3 bg-slate-800/30 border border-white/5 rounded-xl group transition-all"
                            >
                              ...
                              <button
                                onClick={() => removeItem(Number(itemId))}
                                className="p-1.5 hover:text-red-400 text-slate-600"
                              >
            ...
            <div className="h-full overflow-y-auto custom-scrollbar pr-2 pb-32 animate-in fade-in zoom-in-95 duration-300">
              <div className="text-center py-12">
                ...
                <p className="text-slate-500 text-sm mt-2">
                  ตรวจสอบยอดรวมสำหรับการจ่ายให้ทั้ง {selectedShelters.length}{' '}
                  ศูนย์
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto px-4">
                <div className="bg-slate-900/40 border border-white/5 rounded-3xl p-8">
                  <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-6 flex items-center gap-2">
                    <MapPin className="w-4 h-4 text-emerald-500" /> ปลายทาง
                    ({selectedShelters.length})
                  </h3>
                  <ul className="space-y-3">
                    {selectedShelters.map(id => (
                      <li
                        key={`shelter-confirm-${id}`}
                        className="text-sm bg-white/5 p-3 rounded-xl text-slate-300"
                      >
                        {
                          shelters.find(
                ...
                <div className="bg-slate-900/40 border border-white/5 rounded-3xl p-8">
                  <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-6 flex items-center gap-2">
                    <Package className="w-4 h-4 text-blue-500" /> ยอดเบิกสุทธิ
                    (รวมทุกศูนย์)
                  </h3>
                  <ul className="space-y-3">
                    {Object.entries(cart).map(([itemId, qty]) => {
                      ...
                      return (
                        <li
                          key={`summary-${itemId}`}
                          className="flex justify-between items-center bg-white/5 p-3 rounded-xl text-sm text-slate-300"
                        >
                          <span>{item?.name}</span>
        ...
        <div className="fixed bottom-10 right-10 flex items-center gap-3 z-[100]">
          {step > 1 && (
            <button
              ...
              className="p-4 rounded-2xl bg-slate-800/80 hover:bg-red-500 text-slate-400 hover:text-white border border-white/5 transition-all shadow-2xl backdrop-blur-md"
            >
          ...
          <button
            onClick={() => {
              if (step === 1 && selectedShelters.length > 0)
                setStep(2);
              else if (step === 2 && Object.keys(cart).length > 0)
                setStep(3);
          ...
          >
            <span className="text-sm uppercase tracking-widest font-sans">
              {step === 3 ? 'ยืนยันการเบิกจ่าย' : 'ถัดไป'}
            </span>
            {step === 3 ? (
